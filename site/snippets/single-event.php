<div class="xsm-col col-12 sm-col-6 lg-col-3  event-container inline-block border-box p1 mt2">
  <div class="relative drop-shadow event bg-light-gray p1">
    <?php if (strlen($event->desitnation()) < 1): ?>
      <a href="<?= $event->desitnation() ?>" class="absolute top-0 left-0 bottom-0 right-0 z1"></a>
    <?php endif ?>
    <div class="absolute top-0 left-0 bottom-0 right-0 width-bg top-bg bg-image" <?php if($image = $event->image('1.jpg')): ?>style="background-image: url(<?= $image->url() ?>)"<?php endif ?>></div>
    ...
    <div class="bg-faded-bc-white absolute full-width border-box bottom-0 left-0">
      ...
      <div class="bg-bc-off-white px2 py1">
        <a class='dark-gray' href='<?= $event->googlemapslink() ?>'><img src="../images/ui/location.svg" width="8px" class="mr1"><h6 class="caps inline-block m0"><?= $event->location() ?></h6></a>
      </div>
    </div>
  </div>
</div>
